...
class ProductSubcollectionController extends ProductLookupConfigController
{
    protected function productForeignKey(): string
    {
        return 'subcollection_cigam_code';
    }
    ...
    {
        return ProductSubcollection::class;
    }

    protected function viewTitle(): string
    {
        return 'Subcoleções de Produto';
    }

    protected function viewDescription(): string
    {
        return 'Gerencie as subcoleções do cadastro de produtos';
    }

    protected function routeName(): string
    {
        return 'config.product-subcollections';
    }
    ...
    protected function formFields(): array
    {
        return [
            ...
            ['name' => 'name', 'label' => 'Nome', 'type' => 'text', 'required' => true, 'placeholder' => 'Nome da subcoleção'],
    ...
    {
        return [
            'cigam_code' => 'required|string|max:20|unique:product_subcollections,cigam_code'.($isUpdate ? ','.$id : ''),
    ...
    {
        $count = Product::where('subcollection_cigam_code', $model->cigam_code)->count();
        if ($count > 0) {
            return "Esta subcoleção está vinculada a {$count} produto(s) e não pode ser excluída.";
        }
