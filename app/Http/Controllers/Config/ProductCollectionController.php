...
class ProductCollectionController extends ProductLookupConfigController
{
    protected function productForeignKey(): string
    {
        return 'collection_cigam_code';
    }
    ...
    {
        return 'Coleções de Produto';
    }

    protected function viewDescription(): string
    {
        return 'Gerencie as coleções do cadastro de produtos';
    }

    protected function routeName(): string
    {
        return 'config.product-collections';
    }
    ...
    protected function formFields(): array
    {
        return [
            ...
            ['name' => 'name', 'label' => 'Nome', 'type' => 'text', 'required' => true, 'placeholder' => 'Nome da coleção'],
    ...
    {
        return [
            'cigam_code' => 'required|string|max:20|unique:product_collections,cigam_code'.($isUpdate ? ','.$id : ''),
    ...
    {
        $count = Product::where('collection_cigam_code', $model->cigam_code)->count();
        if ($count > 0) {
            return "Esta coleção está vinculada a {$count} produto(s) e não pode ser excluída.";
        }
