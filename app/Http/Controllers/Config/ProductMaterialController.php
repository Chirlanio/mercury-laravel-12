...
class ProductMaterialController extends ProductLookupConfigController
{
    protected function productForeignKey(): string
    {
        return 'material_cigam_code';
    }
    ...
    {
        return 'Materiais de Produto';
    }

    protected function viewDescription(): string
    {
        return 'Gerencie os materiais do cadastro de produtos';
    }

    protected function routeName(): string
    {
        return 'config.product-materials';
    }
    ...
    protected function formFields(): array
    {
        return [
            ...
            ['name' => 'name', 'label' => 'Nome', 'type' => 'text', 'required' => true, 'placeholder' => 'Nome do material'],
    ...
    {
        return [
            'cigam_code' => 'required|string|max:20|unique:product_materials,cigam_code'.($isUpdate ? ','.$id : ''),
    ...
    {
        $count = Product::where('material_cigam_code', $model->cigam_code)->count();
        if ($count > 0) {
            return "Este material está vinculado a {$count} produto(s) e não pode ser excluído.";
        }
