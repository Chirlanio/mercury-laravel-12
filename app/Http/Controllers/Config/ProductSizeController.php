...
class ProductSizeController extends ProductLookupConfigController
{
    protected function productForeignKey(): string
    {
        return 'size_cigam_code';
    }
    ...
    {
        return 'Tamanhos de Produto';
    }

    protected function viewDescription(): string
    {
        return 'Gerencie os tamanhos do cadastro de produtos';
    }
    ...
    protected function formFields(): array
    {
        return [
            ...
            ['name' => 'name', 'label' => 'Nome', 'type' => 'text', 'required' => true, 'placeholder' => 'Nome do tamanho'],
    ...
    {
        return [
            'cigam_code' => 'required|string|max:20|unique:product_sizes,cigam_code'.($isUpdate ? ','.$id : ''),
    ...
    {
        $count = ProductVariant::where('size_cigam_code', $model->cigam_code)->count();
        if ($count > 0) {
            return "Este tamanho está vinculado a {$count} variante(s) de produto e não pode ser excluído.";
        }
