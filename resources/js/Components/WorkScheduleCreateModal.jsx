...
const defaultDays = () => DAY_NAMES.map((_, i) => ({
    day_of_week: i,
    is_work_day: i >= 1 && i <= 5,
    entry_time: i >= 1 && i <= 5 ? '08:00' : '',
    exit_time: i >= 1 && i <= 5 ? '17:48' : '',
    break_start: i >= 1 && i <= 5 ? '12:00' : '',
    break_end: i >= 1 && i <= 5 ? '13:00' : '',
    break_duration_minutes: i >= 1 && i <= 5 ? 60 : 0,
    notes: '',
}));
...
export default function WorkScheduleCreateModal({ isOpen, onClose, onSuccess }) {
    ...
    const handleSubmit = (e) => {
        ...
        router.post('/work-schedules', payload, {
            onSuccess: () => {
                resetForm();
                onSuccess();
            },
    ...
    return (
        <Modal show={isOpen} onClose={handleClose} title="Nova Escala de Trabalho" maxWidth="85vw">
