...
export default function EditModal({ show, onClose, audit, stores = [], onSuccess }) {
    ...
    // Pre-fill form when audit data changes
    useEffect(() => {
        if (audit && show) {
            setData({
                store_id: audit.store_id || '',
                audit_type: audit.audit_type || 'total',
                vendor_id: audit.vendor_id || '',
                audit_cycle_id: audit.audit_cycle_id || '',
                manager_responsible_id: audit.manager_responsible_id || '',
                stockist_id: audit.stockist_id || '',
                random_sample_size: audit.random_sample_size || 10,
                requires_second_count: audit.requires_second_count ?? true,
                requires_third_count: audit.requires_third_count ?? false,
                notes: audit.notes || '',
            });
        }
    }, [audit, show]);
    ...
    const handleSubmit = (e) => {
        ...
        put(route('stock-audits.update', audit.id), {
            onSuccess: () => {
                reset();
                onSuccess?.();
            },
