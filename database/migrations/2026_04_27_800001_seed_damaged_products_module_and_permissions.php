...
/**
 * Seeds the Damaged Products module: central_modules, central_permissions,
 * central_role_permissions, central_pages (apenas as 2 páginas de config —
 * Tipos de Dano e Regras de Marca por Rede), central_menu_page_defaults para
 * essas configs e tenant_modules (Professional + Enterprise).
 *
 * A página principal /damaged-products NÃO é registrada aqui — fica pra Fase 3
 * quando o controller existir (mesma separação que TurnList).
 */
return new class extends Migration
{
    private const MODULE_SLUG = 'damaged_products';
    private const PERMISSION_PREFIX = 'damaged_products.';
    private const CONFIG_PARENT_MENU_NAME = 'Configurações';
    private const CONFIG_PAGES = [
        ['route' => '/config/damage-types',        'name' => 'Tipos de Dano'],
        ['route' => '/config/network-brand-rules', 'name' => 'Regras de Marca por Rede'],
    ];
    ...
    private function seedConfigMenuPageDefaults(): void
    {
        ...
        // MANAGE_DAMAGED_PRODUCTS é dada a super_admin + admin + support, mas
        // os configs (damage_types + network_brand_rules) afetam regras de
        // negócio do matching — só super_admin + admin no menu de Configs.
    ...
    private function addModuleToPlans(): void
    {
        ...
        foreach ($plans as $plan) {
            ...
            // Produtos Avariados disponível em Professional e Enterprise.
            // Starter mantém só os módulos essenciais — gestão de avarias
            // pressupõe múltiplas lojas para que o matching faça sentido.
