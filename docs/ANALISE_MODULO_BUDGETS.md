# Análise do Módulo — Orçamentos (Budgets) · Fases 1 + 2 + 3 + 4

**Data de conclusão da Fase 4:** 20/04/2026
**Commits:** 15 (Fase 1: 5 + Fase 2: 4 + Fase 3: 4 + Fase 4: 2)
**Testes:** 63 feature tests / 273 assertions
**Regressão fundação + Budgets:** 113 tests / 458 assertions
...
| `BudgetConsumptionTest` | 14 | Consumo por item/CC/AC/mês + thresholds + dashboard + items-for-cc |
| `BudgetAlertTest` | 10 | Scan alerts + command + notification channels + recipients filter |

Regressão cruzada: **113 tests** na pilha completa (CC + AC + MC + Budgets).
...
## Fase 4 — Alertas automatizados (CONCLUÍDA 2026-04-20)

Automação diária que transforma o dashboard em ação proativa — o financeiro recebe notification database + mail toda manhã com os CCs/items em alerta, sem precisar abrir o dashboard.

### Entregas

**Backend (1 commit):**
- `BudgetAlertService::scanAlerts(year?)` varre budgets ativos, extrai warning/exceeded por CC e item, agrupa por budget. Só inclui budgets com ao menos 1 alerta.
- `BudgetsAlertCommand` scheduled daily 09:00 (`withoutOverlapping` + log em `storage/logs/budgets-alert.log`):
  - Itera `Tenant::all()->run()` para contexto correto de cada tenant
  - `scanTenant(year, dryRun)` extraído para testabilidade direta (SQLite in-memory)
  - Flags `--year=YYYY` + `--dry-run`
  - `safeLine()` wrapper — funciona tanto com Artisan (output real) quanto instanciado direto em testes (output null)
- `BudgetAlertNotification` (database + mail):
  - Mail rich com grupos warning/exceeded por budget, top-5 CCs de cada (evita email gigante)
  - Action button para `/budgets`
  - Database payload compacto (type, message, affected_scopes, exceeded_total)

**Frontend (1 commit):**
- `AlertsBanner` no topo do `Pages/Budgets/Index.jsx`:
  - Cor vermelha quando há exceeded, amber quando só warning
  - Top-3 budgets em alerta com links diretos para seus dashboards
  - Counter "+N orçamento(s) adicionais" quando truncado
  - Só renderiza se usuário tem `VIEW_BUDGET_CONSUMPTION` E há alertas
- Scan executado sob demanda em `index()` do controller (OK para MVP; futura iteração pode cachear)

### Decisões arquiteturais

#### 1. Thresholds fixos (70% warning / 100% exceeded)
Consistência visual entre dashboard e alerta. Settings por tenant para customizar fica no backlog.

#### 2. Command daily scan, não event-driven
Alternativa: disparar alert ao criar OP que cruza threshold. Escolhido daily por:
- Simplicidade (1 command, 1 notificação/dia/usuário)
- Previsibilidade (user sabe que chega às 09:00)
- Evita flood se várias OPs excedem no mesmo dia
- Gastos de produção mais tarde podem reverter

#### 3. Banner no Index (não widget global)
Mantém contexto — user vê/age quando já está pensando em orçamento.

#### 4. `resolveRecipients()` sem filtro `deleted_at`
Tabela `users` não tem soft delete. `whereNull('deleted_at')` adicionado inicialmente por erro fez os tests falharem com 0 recipients. Fix: remover o filtro.

### Fase 4 — Commits

| Hash | Descrição |
|---|---|
| `c39cd2f` | BudgetAlertService + Command + Notification + 10 tests |
| `(este)` | Banner AlertsBanner no Index + docs |
