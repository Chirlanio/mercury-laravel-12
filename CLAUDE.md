...
- Quadro de fila de atendimento das consultoras no PDV. **Tablet-first** com fullscreen CSS (não API nativa) + drag-and-drop entre painéis via `@dnd-kit/core` (PointerSensor + TouchSensor + KeyboardSensor) — vendedoras usam em tablet na loja física.
- 4 painéis: **Available** (consultoras ativas sem fila/atendimento/pausa), **Queue** (fila ordenada por position), **Attending** (atendimentos ativos com timer), **OnBreak** (pausas com `is_exceeded` calculado, condicional — só renderiza se houver alguém em pausa). Polling silencioso 30s via fetch direto (não Inertia) pra não disparar re-render do shell. Tick 1Hz client-side mantém timers fluidos entre poll cycles.
...
- 2 tipos de pausa default: **Lanche** (15min) e **Almoço** (60min). Migration adicional `2026_04_26_700001_rename_intervalo_to_lanche_break_type` renomeia tenants antigos (idempotente).
- Reuso do enum `TurnListAttendanceStatus` (active/finished) em attendances E breaks — evita criar enum extra.
- **Mutações via axios + 204 No Content** (não Inertia router): controller chama helper `respondOk($request)` que retorna 204 pra XHR (`expectsJson() || ajax()`) e `redirect()->back()` pra fluxo Inertia/form. Frontend usa `window.axios.post` (já com CSRF configurado), elimina re-render dos props da página em cada ação. Após cada POST, `fetchBoard()` no `finally` confirma sucesso ou reverte ao estado real do servidor.
- **Optimistic UI** elimina latência percebida no drag-drop: `apiPost(url, data, optimisticFn)` aplica `setBoard(optimisticFn)` ANTES do POST. 3 helpers puros (`applyOptimisticMove`/`Remove`/`Reorder`) montam o próximo board com defaults (queue_id=-Date.now, position contínua, started_at agora) que o servidor sobrescreve no fetchBoard subsequente. Erros 422 disparam fetchBoard que reverte automaticamente.
- **CSS fullscreen + push layout** (não Fullscreen API nativa): `requestFullscreen()` esconde portais do HeadlessUI (modais Pausa/Finalizar renderizam no `<body>`, fora do elemento solicitado, ficavam invisíveis). Container vira `fixed inset-y-0 right-0 left-0 z-40` em fullscreen. Em telas `lg:`+ aplica `lg:left-16` (sidebar collapsed) ou `lg:left-64` (expanded) — push layout, sidebar permanece visível ao lado em vez de ser coberta. Sincronização sidebar/board via `CustomEvent('sidebar-collapsed-change')` disparado em `AuthenticatedLayout.handleToggleCollapse` + `useEffect` listener no Index. Esc sai do modo.
- **Layout adaptativo dashboard mode**: `h-full overflow-hidden` na container; top row (queue + attending) com `flex-1 min-h-[180px]` toma todo espaço sobrando; bottom panels (Em Pausa, Disponível) com `shrink-0 min-h-[Xpx] max-h-[Xvh]` crescem com o conteúdo até teto VH (Em Pausa 30vh, Disponível 40vh). Grid horizontal interno usa `auto-rows-min` pra cards não esticarem quando há só 1 linha (problema antigo — cards anormalmente altos). `overflow-y-auto` interno cuida de quando conteúdo ultrapassa o teto.
- **Cards: 2 layouts**. **Queue** vertical (`flex-col gap-3 p-4`): topo com avatar+nome+badge `#N` inline, base com 3 botões (Atender/Pausar/Sair) em `grid-cols-3 gap-2 w-full` distribuídos igualmente. **Outros** (Available/Attending/OnBreak) horizontal (`flex-row items-center justify-between gap-3 p-4`): avatar+nome `flex-1 min-w-0` à esquerda, botão único `shrink-0` à direita. Drag handle só na área avatar+nome — botões usam `onPointerDown stopPropagation` pra não iniciar drag. ActionBtn min-h/min-w 44px (Apple HIG).
- **Floating actions** (`fixed right-3 sm:right-4 top-1/2 -translate-y-1/2 z-40`): Atualizar (branco) + Tela cheia/Sair (indigo) sempre acessíveis na borda direita, fora do header. Não competem com título por espaço.
- **Cores semânticas**: Na Fila azul, Atendendo verde, Em Pausa roxo, Disponível cinza. Botões de ação seguem cor do destino (Atender em queue→verde porque vai pra Attending; Finalizar em attending→azul porque volta pra Queue).
- **Responsividade**: Top row `grid-cols-1 md:grid-cols-2 gap-4` (empilha < 768px, lado a lado em iPad portrait+); Disponível/OnBreak `grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4` adapta cards/linha por breakpoint. Seletor de loja gated por `permissions.manage` (não `!isStoreScoped`).
- **Gotchas testáveis**: defaults de status via `$attributes` no Model (sem isso casts enum retornam null em instâncias novas); boundary de meia-noite UTC em testes de período (use `now()->startOfDay()->addHours(10)` em vez de `now()->subMinutes(30)`); `scanTenant()` precisa de `OutputStyle` injetado em testes (chamada direta causa "writeln() on null"); testes do controller usam form-style sem header XHR e recebem `redirect()->back()` legacy (24/24 verdes).
- 83 tests / 273 assertions (Queue 14 + Attendance 11 + Break 8 + Board 8 + Stats 11 + Controller 24 + Cleanup 7).
- **Pendências em backlog**: Config modules pra `MANAGE_TURN_LIST_BREAK_TYPES` e `MANAGE_TURN_LIST_OUTCOMES` (permissions definidas, sem UI); UI de settings (toggle `return_to_position` por loja — endpoint funciona, sem página); notification de pausa excedida; integração Helpdesk; broadcast Reverb realtime em vez de polling; anti-flicker em drag rápido; endpoint API pra dashboards externos. **Backlog detalhado com prompts prontos pra implementação em `docs/turn-list-backlog.md`** (8 itens com contexto, esforço, prioridade e instruções self-contained).
